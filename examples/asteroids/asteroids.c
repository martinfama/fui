...
void init_asteroid_list(asteroids_t *asteroid_list, int max_count, int min_vtx, int max_vtx, float min_speed,
                        float max_speed, float min_x, float max_x, float dx, float min_y, float max_y, float dy) {
    asteroid_list->asteroids = (asteroid_t *)malloc(max_count * sizeof(asteroid_t));
...
void create_asteroid(asteroids_t *asteroid_list) {
    ...
    a->vtx_count = rand() % (asteroid_list->vtx_range[1] - asteroid_list->vtx_range[0]) + asteroid_list->vtx_range[0];
    ...
    a->x = (rand() % 2 == 0) ? asteroid_list->min_x - _dx : asteroid_list->max_x + _dx;
    a->y = (rand() % 2 == 0) ? asteroid_list->min_y - _dy : asteroid_list->max_y + _dy;

    float speed = (float)(rand() % (int)(asteroid_list->speed_range[1] - asteroid_list->speed_range[0])) +
                  asteroid_list->speed_range[0];
    // calculate angle range from asteroid position to the four corners of the screen, and choose a random angle
    // in that range
    float angle_tl = atan2((float)(asteroid_list->min_y - a->y), (float)(asteroid_list->min_x - a->x));
    float angle_tr = atan2((float)(asteroid_list->min_y - a->y), (float)(asteroid_list->max_x - a->x));
    float angle_bl = atan2((float)(asteroid_list->max_y - a->y), (float)(asteroid_list->min_x - a->x));
    float angle_br = atan2((float)(asteroid_list->max_y - a->y), (float)(asteroid_list->max_x - a->x));
...
int update_asteroids(asteroids_t *asteroid_list, float dt, bullet_list_t *bullet_list) {
    int score = 0;
    for (int i = 0; i < asteroid_list->count; i++) {
        asteroid_t *a = &asteroid_list->asteroids[i];
        a->x += a->vx * dt;
        a->y += a->vy * dt;
        if (a->x < asteroid_list->min_x - asteroid_list->dx || a->y < asteroid_list->min_y - asteroid_list->dy ||
            a->x > asteroid_list->max_x + asteroid_list->dx || a->y > asteroid_list->max_y + asteroid_list->dy) {
...
bool collision_detection(asteroids_t *asteroid_list, player_t *player) {
    for (int i = 0; i < asteroid_list->count; i++) {
        ...
        for (int j = 0; j < a->vtx_count; j++) {
            float rotated_x = cosf(player->orientation) * a->vtx_x[j] - sinf(player->orientation) * a->vtx_y[j];
            float rotated_y = sinf(player->orientation) * a->vtx_x[j] + cosf(player->orientation) * a->vtx_y[j];
