...
int main() {
  ...
  float dy = 200.0f;
  init_asteroid_list(asteroid_list, max_asteroid_count, min_vtx, max_vtx,
                     min_speed, max_speed, min_x, max_x, dx, min_y, max_y, dy);
  ...
  while (1) {
    ...
    if (global_keyboard_state.key_state[KEY_UP]) {
      if (powf(player->vx, 2) + powf(player->vy, 2) <
          powf(max_player_speed, 2)) {
    ...
    while (accumulator >= fixed_dt) {
      ...
      update_player(player, fixed_dt, 0.0f, (float)fb.width, 0.0f,
                    (float)fb.height);
      update_bullets(bullet_list, fixed_dt, 0.0f, (float)fb.width, 0.0f,
                     (float)fb.height);
