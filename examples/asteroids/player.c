...
void draw_player(layer_t *layer, player_t *player) {
    ...
    draw_line(layer, player->x + rotated_x1, player->y + rotated_y1, player->x + rotated_x2, player->y + rotated_y2,
              0xFFFF0000);
    draw_line(layer, player->x + rotated_x2, player->y + rotated_y2, player->x + rotated_x3, player->y + rotated_y3,
              0xFFFF0000);
    draw_line(layer, player->x + rotated_x3, player->y + rotated_y3, player->x + rotated_x1, player->y + rotated_y1,
              0xFFFF0000);
}

void update_player(player_t *player, float dt, float min_x, float max_x, float min_y, float max_y) {
...
void shoot(player_t *player) {
    ...
    bullet_list->bullets[bullet_list->count].vx = -sinf(player->orientation) * bullet_list->speed;
    bullet_list->bullets[bullet_list->count].vy = cosf(player->orientation) * bullet_list->speed;
    bullet_list->last_shot = get_time_ms();
    bullet_list->count++;
}

void update_bullets(bullet_list_t *bullet_list, float dt, float min_x, float max_x, float min_y, float max_y) {
