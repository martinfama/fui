...
int main() {
    ...
    while (1) {
        ...
        for (int32_t i = 0; i < num_bodies; i++) {
            ...
            draw_rectangle(&layer_list->layers[0], (int32_t)bodies[i].x - 1, (int32_t)bodies[i].y - 1, 3, 3,
                           bodies[i].color, true);
        }

        draw_circle(&layer_list->layers[0], (int32_t)massive_body.x, (int32_t)massive_body.y,
                    (int32_t)massive_body.r * 0.1f, massive_body.color, false);
