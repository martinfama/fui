...
void draw_mandelbrot(layer_t *layer, int width, int height, float zoom,
                     float move_x, float move_y, int max_iter) {
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      ...
      // uint32_t color = (iter == max_iter) ? 0xFF000000 :
      // hsl_to_rgb((float)iter / max_iter, 1.0f, 0.5f); rescale color with log
      uint32_t color =
          (iter == max_iter)
              ? 0xFF000000
              : hsl_to_rgb((float)log(iter + 1) / log(max_iter), 1.0f, 0.5f);
...
void *mandelbrot_thread(void *args) {
  ...
  draw_mandelbrot(mandelbrot_args->layer, mandelbrot_args->width,
                  mandelbrot_args->height, mandelbrot_args->zoom,
                  mandelbrot_args->move_x, mandelbrot_args->move_y,
                  mandelbrot_args->max_iter);
...
void run_mandelbrot(layer_t *layer, int width, int height, float zoom,
                    float move_x, float move_y, int max_iter) {
  ...
  if (!mandelbrot_running) {
    ...
    pthread_create(&mandelbrot_thread_id, NULL, mandelbrot_thread,
...
int main() {
  ...
  while (1) {
    ...
    int32_t mouse_layer_index = position_layer_location(
        layer_list, global_mouse_state.x, global_mouse_state.y);
    ...
    for (int x = 50; x < 150; x++) {
      draw_line(&layer_list->layers[2], x, 50, x, 150,
                parametrized_rainbow(rainbow_cycle + x * 0.01f));
    }

    draw_circle(&layer_list->layers[0], global_mouse_state.x,
                global_mouse_state.y, 4, 0xFFFF0000, true);
    ...
    if (!mandelbrot_running) {
      ...
      run_mandelbrot(mandelbrot_layer, mandelbrot_args.width,
                     mandelbrot_args.height, mandelbrot_args.zoom,
                     mandelbrot_args.move_x, mandelbrot_args.move_y,
                     mandelbrot_args.max_iter);
    }
    ...
    render_fps(&layer_list->layers[0], 10, layer_list->layers[0].height - 20,
               20.0f);
