...
void play_buffer(int16_t *buffer, int size) { snd_pcm_writei(handle, buffer, size); }
...
void play_sine_note(float frequency, float duration) {
    ...
    for (int i = 0; i < SAMPLE_RATE * duration; i++) {
        ...
        if (i < FADE_DURATION) {
            buffer[i] = (int16_t)(sin(2 * M_PI * frequency * t) * 32767 * (float)i / FADE_DURATION);
        } else if (i > SAMPLE_RATE * duration - FADE_DURATION) {
            buffer[i] =
                (int16_t)(sin(2 * M_PI * frequency * t) * 32767 * (float)(SAMPLE_RATE * duration - i) / FADE_DURATION);
        } else {
...
void play_sine_chord(float *frequencies, int num_frequencies, float duration) {
    ...
    for (int i = 0; i < SAMPLE_RATE * duration; i++) {
        ...
        for (int j = 0; j < num_frequencies; j++) {
            ...
            if (i < FADE_DURATION) {
                buffer[i] += (int16_t)(sin(2 * M_PI * frequencies[j] * t + phase_offset) * 32767 * (float)i /
                                       FADE_DURATION / num_frequencies);
            } else if (i > SAMPLE_RATE * duration - FADE_DURATION) {
                buffer[i] += (int16_t)(sin(2 * M_PI * frequencies[j] * t + phase_offset) * 32767 *
                                       (float)(SAMPLE_RATE * duration - i) / FADE_DURATION / num_frequencies);
            } else {
                buffer[i] += (int16_t)(sin(2 * M_PI * frequencies[j] * t + phase_offset) * 32767 / num_frequencies);
            }
        }
    }
    play_buffer(buffer, SAMPLE_RATE * duration);
...
void play_triangle_note(float frequency, float duration) {
    ...
    for (int i = 0; i < SAMPLE_RATE * duration; i++) {
        ...
        if (i < FADE_DURATION) {
            ...
            buffer[i] = (int16_t)(triangle_wave * 32767 * (float)(SAMPLE_RATE * duration - i) / FADE_DURATION);
        } else {
