...
void load_framebuffer(framebuffer_t *fb, const char *device) {
  ...
  fb_info.real_pixels = mmap(NULL, fb_info.screen_size, PROT_READ | PROT_WRITE,
...
void gray_clear_framebuffer(framebuffer_t *fb, uint8_t intensity) {
  intensity = (0xFF << 24) | (intensity << 16) | (intensity << 8) |
              intensity; // ARGB format
...
void draw_pixel_fb(framebuffer_t *fb, int x, int y, uint32_t color) {
  ...
  fb->pixels[y * fb->line_length_pixels + x] =
...
void sort_layers_z(layer_list_t *list) {
  ...
  for (int i = 0; i < list->count - 1; i++) {
    for (int j = 0; j < list->count - i - 1; j++) {
      if (list->layers[list->z_sorted_index[j]].z_depth >
          list->layers[list->z_sorted_index[j + 1]].z_depth) {
...
void add_layer(layer_list_t *list, int32_t x, int32_t y, int32_t width,
  ...
  layer_t *new_layers =
      realloc(list->layers, (list->count + 1) * sizeof(layer_t));
  if (new_layers == NULL) {
  ...
  memset(list->layers[list->count].pixels, 0,
         width * height * sizeof(uint32_t));
  list->count++;
  list->z_sorted_index =
      realloc(list->z_sorted_index, list->count * sizeof(int32_t));
  if (list->z_sorted_index == NULL) {
...
void clear_layer(layer_list_t *list, int32_t index) {
  ...
  memset(list->layers[index].pixels, 0,
         list->layers[index].width * list->layers[index].height *
             sizeof(uint32_t));
}
...
void draw_layer(layer_list_t *list, int32_t index, framebuffer_t *fb,
  ...
  for (int y = 0; y < layer->height; y++) {
    for (int x = 0; x < layer->width; x++) {
      ...
      if (flush) {
        draw_pixel_fb(fb, layer->x + x, layer->y + y,
                      layer->pixels[y * layer->width + x]);
      } else if (layer->pixels[y * layer->width + x] !=
                 fb->pixels[(layer->y + y) * fb->line_length_pixels +
                            (layer->x + x)]) {
        draw_pixel_fb(fb, layer->x + x, layer->y + y,
                      layer->pixels[y * layer->width + x]);
      }
