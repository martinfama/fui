...
void screenshot_region(const char *filename, framebuffer_t *fb, int32_t x, int32_t y, int32_t width, int32_t height) {
    if (x < 0 || y < 0 || x + width > fb->line_length_pixels || y + height > fb->height) {
    ...
    if (width != fb->width || height != fb->height) {
        for (int32_t row = 0; row < height; row++) {
            memcpy(region_pixels + row * width, fb->real_pixels + (y + row) * fb->line_length_pixels + x,
                   width * sizeof(uint32_t));
        }
    } else
        memcpy(region_pixels, fb->real_pixels, width * height * sizeof(uint32_t));

    // now, we have the issue that for the framebuffer, the order is ARGB, but for the png, it is ABGR
    // so we need to convert the order. we also just set alpha to 0xFF
