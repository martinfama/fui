...
void poll_mouse_events(EventQueue *event_queue) {
  ...
  int rc;
  while ((rc = libevdev_next_event(mouse_dev, LIBEVDEV_READ_FLAG_NORMAL,
                                   &ev)) == LIBEVDEV_READ_STATUS_SUCCESS) {
    if (ev.type == EV_REL) {
      if (ev.code == REL_X || ev.code == REL_Y) {
        MouseMoveEvent move_event = {.timestamp = ev.time.tv_sec * 1000000 +
                                                  ev.time.tv_usec,
        ...
        MouseScrollEvent scroll_event = {.timestamp = ev.time.tv_sec * 1000000 +
                                                      ev.time.tv_usec,
                                         .value = ev.value};
      ...
      if (ev.code == BTN_LEFT || ev.code == BTN_RIGHT ||
          ev.code == BTN_MIDDLE) {
        MouseClickEvent click_event = {.timestamp = ev.time.tv_sec * 1000000 +
                                                    ev.time.tv_usec,
