...
void poll_keyboard_events(EventQueue *event_queue) {
  ...
  int rc;
  while ((rc = libevdev_next_event(keyboard_dev, LIBEVDEV_READ_FLAG_NORMAL,
                                   &ev)) == LIBEVDEV_READ_STATUS_SUCCESS) {
    if (ev.type == EV_KEY) {
      KeyboardEvent keyboard_event = {.timestamp = ev.time.tv_sec * 1000000 +
                                                   ev.time.tv_usec,
                                      .keycode = ev.code,
                                      .value = ev.value};
...
void on_keypress(Event *event) {
  if (event->type == KEYBOARD_EVENT) {
    ...
    if (keyboard_event->value == 1 ||
        keyboard_event->value == 2) { // Key pressed
