...
void print_event_queue(EventQueue *event_queue) {
  for (int i = 0; i < event_queue->count; i++) {
    Event event =
        event_queue->events[(event_queue->head + i) % event_queue->capacity];
    switch (event.type) {
    case EMPTY_EVENT:
    ...
    case MOUSE_CLICK_EVENT:
      pdebugui("Mouse Click Event: Timestamp: %lld, X: %d, Y: %d, Button: %s\n",
               event.data.click.timestamp, event.data.click.x,
               event.data.click.y,
               libevdev_event_code_get_name(EV_KEY, event.data.click.button));
      break;
    case MOUSE_MOVE_EVENT:
      pdebugui("Mouse Move Event: Timestamp: %lld, DX: %d, DY: %d\n",
               event.data.move.timestamp, event.data.move.dx,
               event.data.move.dy);
      break;
    case MOUSE_SCROLL_EVENT:
      pdebugui("Mouse Scroll Event: Timestamp: %lld, Value: %d\n",
               event.data.scroll.timestamp, event.data.scroll.value);
      break;
    case KEYBOARD_EVENT:
      pdebugui(
          "Keyboard Event: Timestamp: %lld, Keycode: %s, Value: %d\n",
          event.data.keyboard.timestamp,
          libevdev_event_code_get_name(EV_KEY, event.data.keyboard.keycode),
          event.data.keyboard.value);
      break;
    default:
