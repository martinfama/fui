...
void draw_rectangle(layer_t *layer, int x, int y, int width, int height,
  ...
  if (x + width < 0 || x >= layer->width || y + height < 0 ||
      y >= layer->height) {
    return;
  }
...
void draw_circle(layer_t *layer, int x, int y, int radius, uint32_t color,
                 bool filled) {
  if (x + radius < 0 || x - radius >= layer->width || y + radius < 0 ||
      y - radius >= layer->height) {
    return;
  }
...
void draw_aa_line(layer_t *layer, int x1, int y1, int x2, int y2,
  ...
  while (true) {
    ...
    if (2 * e2 >= -dx) {
      ...
      if (e2 + dy < ed) {
        uint32_t temp_color =
            (color & 0x00FFFFFF) | ((255 * (e2 + dy) / ed) << 24);
        draw_pixel(layer, x1, y1 - sy,
                   temp_color); // Adjust for inverted y-coordinates
      }
      err -= dy;
      x1 += sx;
    }
    if (2 * e2 <= dy) {
      ...
      if (dx - e2 < ed) {
        uint32_t temp_color =
            (color & 0x00FFFFFF) | ((255 * (dx - e2) / ed) << 24);
